...
row_counts = spark.sql("""
    SELECT 'bronze_ecommerce_events'          AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.bronze_ecommerce_events
    UNION ALL
    SELECT 'bronze_user_profiles_raw'         AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.bronze_user_profiles_raw
    UNION ALL
    SELECT 'silver_ecommerce_events'          AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.silver_ecommerce_events
    UNION ALL
    SELECT 'silver_user_profiles'             AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.silver_user_profiles
    UNION ALL
    SELECT 'silver_enriched_purchases'        AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.silver_enriched_purchases
    UNION ALL
    SELECT 'silver_session_summary'           AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.silver_session_summary
    UNION ALL
    SELECT 'gold_post_transaction_triggers'   AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.gold_post_transaction_triggers
    UNION ALL
    SELECT 'gold_user_targeting_profile'      AS layer_table, COUNT(*) AS rows FROM ius_unity_prod.sandbox.gold_user_targeting_profile
...
profile_cdc_check = spark.sql("""
    ...
    FROM ius_unity_prod.sandbox.silver_user_profiles
""")
...
session_funnel = spark.sql("""
    ...
    FROM ius_unity_prod.sandbox.silver_session_summary
    GROUP BY converted
...
behavioral_tiers = spark.sql("""
    ...
    FROM ius_unity_prod.sandbox.gold_user_targeting_profile
